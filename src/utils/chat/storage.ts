...
export const STORAGE_KEYS = {
  ...
  PENDING_ATTACHMENTS_SMALL: "pendingAttachmentsSmall",
  PENDING_ATTACHMENTS_LARGE: "hasLargeAttachments",
} as const;
...
async function smartSaveAttachments(files: File[]): Promise<void> {
  ...
  if (smallFiles.length > 0) {
    sessionStorage.setItem(
      STORAGE_KEYS.PENDING_ATTACHMENTS_SMALL,
      JSON.stringify(smallFiles),
    );
  }
  ...
  if (largeFiles.length > 0) {
    ...
    sessionStorage.setItem(STORAGE_KEYS.PENDING_ATTACHMENTS_LARGE, "true");
  }
}
...
async function smartGetAttachments(): Promise<File[]> {
  ...
  const smallFilesJson = sessionStorage.getItem(
    STORAGE_KEYS.PENDING_ATTACHMENTS_SMALL,
  );
  if (smallFilesJson) {
  ...
  const hasLargeFiles = sessionStorage.getItem(
    STORAGE_KEYS.PENDING_ATTACHMENTS_LARGE,
  );
  if (hasLargeFiles) {
...
export async function clearPendingMessage(): Promise<void> {
  ...
  sessionStorage.removeItem(STORAGE_KEYS.PENDING_ATTACHMENTS_SMALL);
  sessionStorage.removeItem(STORAGE_KEYS.PENDING_ATTACHMENTS_LARGE);
